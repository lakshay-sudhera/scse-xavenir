...
export async function generateCertificatesForEvent(
  ...
  const allUserIDs = [...new Set(registrations.flatMap((r) => r.members))];
  ...
  // ── Step 1: Generate winner / runner_up certs first ──
  for (const w of winners) {
    try {
      ...
      const imageBuffer = await buildCertificateImage(user.fullName, eventName, w.type, w.position, w.userID);
      const url = await uploadCertificateToCloudinary(imageBuffer, w.userID, eventName, w.type);
      await CertificateModel.create({ userID: w.userID, eventName, type: w.type, position: w.position, certificateUrl: url });
      result.success++;
    } catch (err: any) {
      result.failed++;
  ...
  // ── Step 2: Participation certs for everyone else ──
